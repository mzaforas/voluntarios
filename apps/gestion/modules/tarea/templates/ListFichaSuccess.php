...
<div id="tabs">
   <ul>
     ...
     <li><a href="#equipos"><span>Asignar equipos</span></a></li>
   </ul>
   ...
   <div id="equipos">
     ...
     <table>
     <thead>
     <tr><th>Equipo</th><th>Nombre</th><th>Número miembros</th><th>Perfiles asociados</th><th>Tareas asignadas</th><th>Desasignar</th></tr>
     <thead>
     <tbody>
     ...
     <td><?php echo link_to('Desasignar','tarea/desasignarEquipo?equipo_id='.$equipo_asignado->getId().'&tarea_id='.$tarea->getId(),array('title'=>'Desasignar equipo')); ?></td>
     </tr>
     <?php endforeach; ?>
     </tbody>
     </table>

     <h3>Equipos disponibles no asignados</h3>
     <table>
     <thead>
     <tr><th>Equipo</th><th>Nombre</th><th>Número miembros</th><th>Perfiles asociados</th><th>Tareas asignadas</th><th>Asignar</th></tr>
     <thead>
     <tbody>
     <?php foreach ($equipos_disponibles as $equipo_disponible): ?>
     <tr>
     <td><?php echo link_to($equipo_disponible,'equipo/ListFicha?id='.$equipo_disponible->getId(),array('title'=>'Ficha del equipo')); ?></td>
     <td><?php echo $equipo_disponible->getNombre(); ?></td>
     <td><?php echo $equipo_disponible->getVoluntarios()->count(); ?></td>
     <td><?php foreach ($equipo_disponible->getPerfiles() as $perfil) { echo $perfil; } ?></td>
     <td><?php echo $equipo_disponible->getTareas()->count(); ?></td>
     <td><?php echo link_to('Asignar','tarea/asignarEquipo?equipo_id='.$equipo_disponible->getId().'&tarea_id='.$tarea->getId(),array('title'=>'Asignar equipo')); ?></td>
     </tr>
     <?php endforeach; ?>
     </tbody>
     </table>
...
<ul class="sf_admin_actions">
  ...
  <li class="sf_admin_action_list"><?php echo link_to('Descargar PDF','tarea/generarPdf?id='.$tarea->getId()) ?>
</li>
